...
function makeBuddyList(
  ...
  function getBuddyListFriends(list: BuddyList): Friend[] {
    return list.members.reduce<Friend[]>((friends, buddy) => {
      // Eğer buddy bir Friend ise, diziye ekle
      if ("phone" in buddy) {
        friends.push(buddy);
      }
      return friends; // Akümülatörü döndür
    }, []); // Boş bir dizi ile başla
  }

  const friendsInFootballBuddies = getBuddyListFriends(myFootballBuddies);
  console.log("Friends in my football buddies:", friendsInFootballBuddies);
